...
types of healthcare institutions that exist in Istanbul. 

The code produces visualizations in both Turkish and English.

The visualization will be a bar chart that has healthcare institution
...
inst_types_counts = health.loc[:,"institution_type"].value_counts()
...
labels = list(health.loc[:,"institution_type_abbrv_eng"].value_counts().index)
...
ax.set_title(#"In Istanbul, there are " + r'\textcolor{red}{3522}' + " healthcare institutions distributed across " + r'\textcolor{red}{33}' + " categories.",
             "In Istanbul, there are 3522 healthcare institutions distributed across 33 categories.",
             fontdict = font_title,
             pad = 20)
...
#Get labels for x - axis ticks
labels = list(health.loc[:,"institution_type_abbrv_tr"].value_counts().index)
...
ax.set_title("İstanbul'da 33 farklı kategoriye dağılmış 3522 tane sağlık kuruluşu var.",
             fontdict = font_title,
             pad = 20)


#Add x axis label
ax.set_xlabel(u"Sağlık kuruluşu türü",
              fontdict = font_axislabels,
              labelpad= 18)

# Add y axis label
ax.set_ylabel(u"Sağlık kuruluşu sayısı",
              fontdict = font_axislabels,
              labelpad= 18)
...
plt.savefig(export_path, format = "png", dpi = 1200, bbox_inches="tight")

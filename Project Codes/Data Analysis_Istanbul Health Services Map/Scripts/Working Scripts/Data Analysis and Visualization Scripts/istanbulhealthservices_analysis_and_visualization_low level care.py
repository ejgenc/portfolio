...
low_level_mask = health.loc[:,"care_type"] == "low level"
...
district_inst_count = health_low_level.loc[:,"district_tr"].value_counts().rename_axis("district_tr").reset_index(name = "count")
...
r1 = st.pearsonr(districts_with_inst_count["population"],
                 districts_with_inst_count["count"])[0]

#Does this correlate with income ?

r2 = st.pearsonr(districts_with_inst_count["yearly_average_household_income"],
                 districts_with_inst_count["count"])[0]


#Distribution across district normalized by population
districts_with_inst_count.loc[:,"normalized_count"] = districts_with_inst_count.loc[:,"count"] / districts_with_inst_count.loc[:,"population"]
...
#Get labels for x - axis ticks
labels = list(districts_with_inst_count.sort_values(by = "count", ascending = False).loc[:,"district_tr"])
...
cbar.set_label('İlk adım sağlık kuruluşu sayısı',
                size = 8,
                weight = "bold")
...
# Add y axis label
ax_2.set_ylabel("İlk adım sağlık kuruluşu sayısı",
              fontdict = font_axislabels,
              labelpad= 18)
...
ax_3.set_title("İlk adım sağlık kuruluşlarının farklı değişkenler ile korelasyonu:",
               color = "black",
               weight = "bold",
               fontsize = 14,
               pad = 15)
...
ax_4.set_xlabel("Yıllık ortalama hanehalkı geliri",
              fontdict = font_axislabels,
              labelpad= 2)
...
#Set figure title
fig.suptitle("İstanbul'da ilk adım sağlık kuruluşları \n nüfüs ile orantılı bir şekilde dağılmış durumda." ,
              family = 'sans-serif',
              fontname = "Arial",
              color =  'black',
              weight = 'bold',
